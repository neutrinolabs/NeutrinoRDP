...
static tbool tsmf_pulse_connect(TSMFPulseAudioDevice* pulse)
{
	pa_context_state_t state;
...
static tbool tsmf_pulse_open(ITSMFAudioDevice* audio, const char* device)
{
...
static tbool tsmf_pulse_close_stream(TSMFPulseAudioDevice* pulse)
{
...
static tbool tsmf_pulse_open_stream(TSMFPulseAudioDevice* pulse)
{
	pa_stream_state_t state;
...
static tbool tsmf_pulse_set_format(ITSMFAudioDevice* audio,
...
static tbool tsmf_pulse_play(ITSMFAudioDevice* audio, uint8* data, uint32 data_size)
{
	TSMFPulseAudioDevice* pulse = (TSMFPulseAudioDevice*) audio;
	uint8* src;
	int len;
