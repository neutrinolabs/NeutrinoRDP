 ...
 * 	lockedConference		BOOL-EAN,
 * 	listedConference		BOOL-EAN,
 * 	conductibleConference		BOOL-EAN,
...
tbool gcc_read_conference_create_request(STREAM* s, rdpSettings* settings)
{
	uint16 length;
	uint8 choice;
	uint8 number;
...
tbool gcc_read_conference_create_response(STREAM* s, rdpSettings* settings)
{
	uint16 length;
	uint32 tag;
	uint16 nodeID;
...
tbool gcc_read_client_data_blocks(STREAM* s, rdpSettings* settings, int length)
{
	uint16 type;
	uint16 blockLength;
	int pos;
...
tbool gcc_read_user_data_header(STREAM* s, uint16* type, uint16* length)
{
...
tbool gcc_read_client_core_data(STREAM* s, rdpSettings* settings, uint16 blockLength)
{
	uint32 version;
	uint16 colorDepth = 0;
...
tbool gcc_read_server_core_data(STREAM* s, rdpSettings* settings)
{
	uint32 version;
...
tbool gcc_read_client_security_data(STREAM* s, rdpSettings* settings, uint16 blockLength)
{
	if (blockLength < 8)
		return false;
...
tbool gcc_read_server_security_data(STREAM* s, rdpSettings* settings)
{
	uint8* data;
	uint32 length;
...
tbool gcc_read_client_network_data(STREAM* s, rdpSettings* settings, uint16 blockLength)
{
	int i;
...
tbool gcc_read_server_network_data(STREAM* s, rdpSettings* settings)
{
	int i;
	uint16 MCSChannelId;
	uint16 channelCount;
...
tbool gcc_read_client_cluster_data(STREAM* s, rdpSettings* settings, uint16 blockLength)
{
	uint32 flags;
...
tbool gcc_read_client_monitor_data(STREAM* s, rdpSettings* settings, uint16 blockLength)
{
	printf("CS_MONITOR\n");
	return true;
}
