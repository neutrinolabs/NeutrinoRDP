...
tbool license_send(rdpLicense* license, STREAM* s, uint8 type)
{
	int length;
	uint8 flags;
	uint16 wMsgSize;
...
tbool license_recv(rdpLicense* license, STREAM* s)
{
	uint16 length;
	uint16 channelId;
	uint16 sec_flags;
...
tbool license_send_valid_client_error_packet(rdpLicense* license)
{
	STREAM* s;
